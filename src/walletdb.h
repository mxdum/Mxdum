...
public:
    ...
    bool WriteZMDMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMDMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMDMSeed_deprecated(uint256& seed);
    bool EraseZMDMSeed();
    bool EraseZMDMSeed_deprecated();

    bool WriteZMDMCount(const uint32_t& nCount);
    bool ReadZMDMCount(uint32_t& nCount);
