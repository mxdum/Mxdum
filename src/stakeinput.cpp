...
// The zMDM block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMDM is a hash of the serial
...
{
    //Create an output returning the zMDM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMDMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMDM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMDM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMDMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMDM output", __func__);
...
{
    CzMDMTracker* zmdmTracker = pwallet->zmdmTracker.get();
    CMintMeta meta;
    if (!zmdmTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmdmTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MDM stake is the outpoint
