...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zmdmTracker = unique_ptr<CzMDMTracker>(new CzMDMTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZMDM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZMDM_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
