...
#ifndef Mxdum_ZMDMWALLET_H
#define Mxdum_ZMDMWALLET_H
...
public:
    CzMDMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMDM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMDM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Mxdum_ZMDMWALLET_H
