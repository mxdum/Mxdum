...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MDM);
    ...
    if (!settings.contains("nAnonymizeMxdumAmount"))
        settings.setValue("nAnonymizeMxdumAmount", 1000);

    nAnonymizeMxdumAmount = settings.value("nAnonymizeMxdumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMxdumAmount"))
        SoftSetArg("-anonymizemxdumamount", settings.value("nAnonymizeMxdumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mxdum.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMxdumAmount:
            return QVariant(nAnonymizeMxdumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMxdumAmount:
            nAnonymizeMxdumAmount = value.toInt();
            settings.setValue("nAnonymizeMxdumAmount", nAnonymizeMxdumAmount);
            emit anonymizeMxdumAmountChanged(nAnonymizeMxdumAmount);
            break;
        case CoinControlFeatures:
