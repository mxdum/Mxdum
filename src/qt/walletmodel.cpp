...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered mxdum address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 MDM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal mxdum:URI (mxdum:XyZ...?message=example)
            {
...
static void NotifyzMDMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMDMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMDMReset.connect(boost::bind(NotifyzMDMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMDMReset.disconnect(boost::bind(NotifyzMDMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmdmTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
