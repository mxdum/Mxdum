...
CzMDMWallet::CzMDMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmdm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMDMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMDMSeed_deprecated()) {
                    LogPrintf("%s: Updated zMDM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmdm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmdm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMDMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMDMCount(nCountLastUsed);
    else if (!walletdb.ReadZMDMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMDMWallet::Lock()
{
    seedMaster = 0;
}

void CzMDMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMDMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMDM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMDMWallet::LoadMintPoolFromDB()
{
...
void CzMDMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMDMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMDMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmdmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzMDMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMDM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmdmTracker which also adds to database
    pwalletMain->zmdmTracker->Add(dMint, true);
...
void CzMDMWallet::SeedToZMDM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMDMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMDMWallet::GenerateDeterministicZMDM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMDMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMDMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
